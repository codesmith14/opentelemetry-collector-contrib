...
func (r collectDRecord) extractAndAppendMetrics(metrics []*metricspb.Metric, defaultLabels map[string]string) ([]*metricspb.Metric, error) {
	...
	if r.Time != nil && r.Severity != nil && r.Message != nil {
		return metrics, nil
	}
...
func (r collectDRecord) metricType(dsType *string, isDouble bool) metricspb.MetricDescriptor_Type {
	...
	// TODO: check which type to use for absolute. Prometheus collectd exporter just ignores it
	// https://github.com/prometheus/collectd_exporter/blob/master/main.go#L109-L129
	case "gauge", "absolute", "":
		if isDouble {
