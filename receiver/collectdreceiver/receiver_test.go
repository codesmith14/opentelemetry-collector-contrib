...
func TestCollectDServer(t *testing.T) {
	...
	logger := zap.NewNop()
	cdr, err := New(logger, endpoint, defaultTimeout, "", sink)
	if err != nil {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("POST", "http://"+endpoint, bytes.NewBuffer([]byte(tt.requestBody)))
...
func (m *mockMetricsSink) ConsumeMetricsData(ctx context.Context, md consumerdata.MetricsData) error {
	m.receivedData = append(m.receivedData, md)
	m.wg.Done()
	return nil
}
...
func assertMetricsAreEqual(t *testing.T, metrics1, metrics2 []*metricspb.Metric) {
	...
	for i := 0; i < len(metrics1); i++ {
		...
		if len(md1.LabelKeys) != len(md1.LabelKeys) {
